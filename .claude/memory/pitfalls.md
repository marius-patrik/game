...
## `.claude/worktrees/laughing-wescoff-669d84` is a docs-only placeholder, NOT a git worktree
This directory holds only `.placeholder` + `.claude/launch.json` (the preview-cwd shim). It has no `.git` of its own. When the Bash tool's cwd defaults here, `git` commands walk UP to `/Users/user/Documents/projects/game/.git` and operate on the PRIMARY checkout. A naive `git reset --hard origin/main` from laughing-wescoff silently advances the primary's HEAD. Symptom: preview starts serving whatever main points at (which might be your feature branch if you just switched). Worth remembering in any future session — the laughing-wescoff folder exists only to satisfy the harness's cached-cwd assertion, not to track code.

## Claude.app `disclaimer` helper gates `preview_start` child processes on macOS
On some hosts the Claude.app install wraps every new `bun run …` child spawned by `preview_start` with `/Applications/Claude.app/Contents/Helpers/disclaimer`. Until the user manually accepts the dialog, the wrapped `bun` never execs. Symptoms: `preview_logs` returns "No logs yet", `curl :3000` exits 7, `preview_screenshot` returns a black frame, process state is `Ss` (stopped). A pure-execution agent cannot complete live preview loops under this gate — document the gap in the PR body and let the overseer verify post-merge once the disclaimer has been dismissed once. Separately: if the preview's cached cwd points at a worktree you removed, `shell-init: error retrieving current directory` surfaces — fix by re-creating the worktree with `git worktree add -f <path> --detach <ref>` + `bun install`.

## When BOTH Claude and Codex are rate-limited, preserve WIP and stop
In a single session, if you burn through Claude's usage cap AND Codex's usage cap (both happen within a 5-hour rolling window under heavy parallel dispatch), you can still push existing WIP branches to origin but cannot dispatch new agents. Don't try to resolve complex rebases manually from the overseer seat — that burns remaining context and often leaves broken state. Prefer: commit worktree WIP to a `wip/<slug>` branch, add a status comment to the blocked PR, update work.md Now with the blocker, then `/flush` + hand off to a fresh session after the rate-limit window. The fresh session (with restored budgets) can re-dispatch the resolver in one shot.

## Surface-level `sed` merge of `<<<<<<< HEAD`/`=======`/`>>>>>>>` is fragile
When a rebase has many conflicts, stripping conflict markers with `sed` (keeping both halves additively) handles ~70% of cases but breaks on: duplicate imports (compile-time `TS2300`), double-returns in the same function (`TS2657: JSX expressions must have one parent element`), missing close-braces on handlers (`TS1005: ',' expected` / `TS1472: 'catch' expected`). The `sed` approach also swallows the structural `});` that sometimes appears only on one side of the conflict. Prefer: for conflicts in one or two files, hand-resolve; for rebases spanning 5+ files with semantic divergence, dispatch a dedicated Codex resolver with explicit per-file fix guidance in the prompt. Do NOT attempt semantic multi-file rebases from the overseer seat.
