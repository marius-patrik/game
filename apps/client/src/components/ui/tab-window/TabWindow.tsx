...
export function TabWindow({
  ...
  const tabDefinitions = useMemo(() => parseTabs(children), [children]);
  const tabSignature = useMemo(
    () => tabDefinitions.map((tab) => tab.id).join("|"),
    [tabDefinitions],
  );
  const layoutConfig = useMemo<LayoutConfig>(
    () => ({
      defaultWindowId,
      tabIds: tabSignature.length > 0 ? tabSignature.split("|") : [],
    }),
    [defaultWindowId, tabSignature],
  );
  ...
  const layout = useMemo(
    () => reconcileLayout(persistedLayout, layoutConfig),
    [layoutConfig, persistedLayout],
  );

  useEffect(() => {
    initializeLayout(id, layoutConfig);
  }, [id, initializeLayout, layoutConfig]);
