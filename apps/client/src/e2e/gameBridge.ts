...
type MoveTarget = { x: number; z: number };
...
let latestSnapshotOwnerId: string | null = null;
let projector: Projector | null = null;
let moveSender: MoveSender | null = null;
...
export function publishGameE2ESnapshot(input: {
  ...
  installBridge();
  latestSnapshotOwnerId = input.ownerId ?? null;
  latestSnapshot = {
    status: input.status,
    error: input.error,
    zoneId: input.zoneId,
...
export function clearGameE2ESnapshot(ownerId?: string) {
  if (!isGameE2EEnabled()) return;
  if (ownerId && latestSnapshotOwnerId && latestSnapshotOwnerId !== ownerId) return;
  latestSnapshotOwnerId = null;
  latestSnapshot = null;
}
...
export function setGameE2EMoveSender(next: MoveSender | null) {
  if (!isGameE2EEnabled()) return;
  installBridge();
  moveSender = next;
}
